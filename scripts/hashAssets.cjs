...
var import_tiny_glob = __toModule(require_tiny_glob());
...
async function main() {
  const assetFiles = await (0, import_tiny_glob.default)(`${assetsDir}/{css,js}/**/*.{css,js}`);
  ...
  for (const filePath of assetFiles) {
    ...
    const newFilePath = import_path.default.format(__spreadProps(__spreadValues({}, parsedPath), {
      base: void 0,
      ext: "." + hash + parsedPath.ext
    }));
    import_fs.default.renameSync(filePath, newFilePath);
    manifest[trimIndex(filePath)] = trimIndex(newFilePath);
  }
  import_fs.default.writeFileSync(`${assetsDir}/manifest.json`, JSON.stringify(manifest, null, 2));
